...
contract Draqle is Ownable {
    ...
    mapping(address => uint256) withdrawTime;
    ...
    function deposit(
        ...
        require(_amount >= 10**19, "You must deposit more than 10 usdc");
        ...
        if (referal == true) referalFeeAmount = referalFee;
        usdcToken.safeTransferFrom(
            msg.sender,
            treasuryWallet,
            _amount.mul(100 - referalFeeAmount - developerFee).div(100)
        );
        usdcToken.safeTransferFrom(
            msg.sender,
            deveoperWallet,
            _amount.mul(developerFee).div(100)
        );
        if (referal == true) {
            usdcToken.safeTransferFrom(
                msg.sender,
                referedAddress,
                _amount.mul(referalFee)
            );
        }
    ...
    function compound() external nonReentrant {
        ...
        require(
            withdrawTime[msg.sender] + 24 hours > block.timestamp,
            "Cooldown is not ready yet"
        );
        uint256 reward = miners[msg.sender].div(100);
        miners[msg.sender].add(reward);
        withdrawTime[msg.sender] = block.timestamp;
    }
    ...
    function getMinerCount() external view returns (uint256) {
        return miners[msg.sender].div(10**18);
    }
}
/*
0x166E1A6277083a0F385fc47Fae40f7b7193f91b9 bsc testnet address

delta testnet token : 0xe2cD35727D8bb9b0b8b3F3CD64AC295B0Acc6Be8
 */
